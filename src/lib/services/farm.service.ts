...
export class FarmService {
  ...
  static async getFarmData(startDate: Date, endDate: Date) {
    ...
    const [totalAnimals, allCowNumbersObj, milkRecords, events] = await Promise.all([
      prisma.cow.count(),
      prisma.cow.findMany({ select: { number: true } }),
      prisma.milkRecord.findMany({
        where: { date: { gte: startOfTargetDate, lte: endOfTargetDate } },
        orderBy: { date: "desc" }
      }),
