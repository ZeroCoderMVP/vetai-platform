...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
     const group = await prisma.groupUnit.findUnique({ where: { id } });
     if (!group) return NextResponse.json({ success: false, error: "Группа не найдена" }, { status: 404 });
