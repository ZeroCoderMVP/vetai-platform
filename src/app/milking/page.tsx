...
export default function MilkingPage() {
  ...
  const [historySource, setHistorySource] = useState<"afi" | "aic">("afi");
    ...
    <AppLayout title="Учёт молока" alertCount={kpi.herdAlerts}>
      ...
      <div className="kpi-grid">
        <div className="kpi-card green">
          <div className="kpi-header">
            <span className="kpi-label">
              {activeTab === "live" ? "Общий надой (сессия)" : "Валовый надой (AFI)"}
            </span>
          ...
          <div className="kpi-value">
            {activeTab === "live"
              ...
              : (dbData?.summary?.afiTotalYield || 0).toLocaleString("ru-RU")}
            <span className="kpi-unit">кг</span>
          </div>
          {activeTab === "history" && dbData?.summary?.totalYield > 0 && (
             <div className="mt-1 text-xs text-gray-500">
               Детализация AIC: {(dbData.summary.totalYield || 0).toLocaleString("ru-RU")} кг
             </div>
          )}
        </div>
        ...
        <div className="kpi-card purple">
          <div className="kpi-header">
            <span className="kpi-label">
              {activeTab === "live" ? "Коров подоено" : "Дойное поголовье"}
            </span>
          ...
          {activeTab === "history" && dbData?.summary?.cowCount > 0 && (
             <div className="mt-1 text-xs text-gray-500" style={{ fontSize: 11, opacity: 0.7, marginTop: 4 }}>
               В среднем за день
             </div>
          )}
        </div>
      ...
      {activeTab === "live" ? (
        ...
        <>
          {Object.entries(byMilking).sort(([a], [b]) => Number(a) - Number(b)).map(([milkingNum, records]) => (
            <MilkingSessionCard 
                key={milkingNum} 
                milkingNum={milkingNum} 
                records={records} 
                milkingLabels={milkingLabels} 
            />
          ))}
        </>
      ) : (
        ...
        <div className="card">
          <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
            ...
            <div style={{ display: "flex", gap: "var(--space-2)" }}>
              <button
                className={`badge ${historySource === "afi" ? "badge-success" : "badge-neutral"}`}
                style={{ cursor: "pointer", border: "none", outline: "none", padding: "4px 12px" }}
                onClick={() => setHistorySource("afi")}
              >
                AFI (Суточные: {dbData?.afiRecords?.length || 0})
              </button>
              <button
                className={`badge ${historySource === "aic" ? "badge-success" : "badge-neutral"}`}
                style={{ cursor: "pointer", border: "none", outline: "none", padding: "4px 12px" }}
                onClick={() => setHistorySource("aic")}
              >
                AIC (Сессии: {dbData?.pagination?.total || 0})
              </button>
            </div>
          </div>
          <div className="card-body" style={{ padding: 0 }}>
            {historySource === "afi" ? (
              dbData?.afiRecords && dbData.afiRecords.length > 0 ? (
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th>Надой Утро, кг</th>
                        <th>Надой День, кг</th>
                        <th>Надой Вечер, кг</th>
                        <th>Итого за сутки, кг</th>
                      </tr>
                    </thead>
                    <tbody>
                      {dbData.afiRecords.map((r: any, i: number) => (
                        <tr key={i}>
                          <td style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>
                            {new Date(r.date).toLocaleDateString("ru-RU")}
                          </td>
                          <td><strong>#{r.cowNumber}</strong></td>
                          <td>{r.actualSession1 > 0 ? r.actualSession1.toFixed(1) : "—"}</td>
                          <td>{r.actualSession2 > 0 ? r.actualSession2.toFixed(1) : "—"}</td>
                          <td>{r.actualSession3 > 0 ? r.actualSession3.toFixed(1) : "—"}</td>
                          <td><strong>{r.actualTotal > 0 ? r.actualTotal.toFixed(1) : "—"}</strong></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="empty-state" style={{ padding: "var(--space-8)" }}>
                  <div className="empty-state-icon">📭</div>
                  <div className="empty-state-text">Нет данных AFI DAY_MILK за выбранный период.</div>
                </div>
              )
            ) : (
    ...
    <div className="card" style={{ marginBottom: "var(--space-4)" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 14, opacity: 0.7 }}>{expanded ? "▼" : "▶"}</span>
              ...
              <tbody>
                {records.sort((a: any, b: any) => b.yield - a.yield).map((r: any, i: number) => (
                  <tr key={i}>
                    ...
                    <td>{r.stall}</td>
                    <td style={{ fontFamily: "var(--font-mono)", fontSize: 12 }}>
                      {r.time ? `${r.time.substring(0, 2)}:${r.time.substring(2, 4)}` : "—"}
                    </td>
